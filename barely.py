...
def tokenize(contents):
    ...
    for index, character in enumerate(contents):
        if character == ' ' and not in_quotes and not in_comment:
            tokens.extend(tokenize_small(buffer))
            buffer = ""
        elif character == '(' and not in_quotes and not in_comment:
            tokens.extend(tokenize_small(buffer))
            ...
            buffer = ""
        elif character == '"' and not in_comment:
            if in_quotes:
                ...
                in_quotes = True
        elif character == '/' and contents[index + 1] == '/':
            in_comment = True
            tokens.extend(tokenize_small(buffer))
            tokens.append(ColonToken())
            buffer = ""
...
def get_statement_c(tokens, index):
    ...
    if isinstance(token, KeywordToken):
        if token.word == "function":
            ...
            if has_body and (len(current_function.instructions) == 0 or not isinstance(current_function.instructions[-1], ReturnNode)):
            ...
            if not isinstance(tokens[index + i - 1], SemiColonToken):
                ...
                statement0 = [ZeroNode(variables[names[0]]), AssignNode(names[0])]
                index += i - 1
            ...
            for instruction in function.instructions:
                if isinstance(instruction, DeclareNode):
                    ...
                    if not popped == "boolean":
                            ...
                            exit()
                elif isinstance(instruction, ZeroNode):
                    types.append(instruction.type)
                else:
        ...
        if isinstance(function, FunctionNode):
            ...
            for index0, instruction in enumerate(function.instructions):
                if isinstance(instruction, InvokeNode):
                    ...
                    if remove_invalid_linux_x86_64(called_name) in struct_things:
                    ...
                    elif instruction.name == "@syscall0":
                        contents += "pop rax\n"
                        contents += "syscall\n"
                        contents += "push rax\n"
                        ...
                        contents += "sub rsp, 1\n"
                        contents += "mov [rsp], al\n"
                    elif instruction.name == "byte_to_integer":
                        contents += "mov rax, 0\n"
                        contents += "mov al, [rsp]\n"
                        contents += "add rsp, 1\n"
                        contents += "push rax\n"
                        ...
                        if isinstance(function.instructions[index0 + 1], PointerNode):
                            ...
                            while j < size:
                                if size - j >= 8:
                                    ...
                                    contents += "mov al, [rbp+" + str(16 + location + j) + "]\n"
                                    contents += "mov [rsp+" + str(j) + "], al\n"
                                    j += 1
                                else:
                                    print("sizing error")
                                    exit()
                        ...
                        if isinstance(function.instructions[index0 + 1], PointerNode):
                            ...
                            while j < size:
                                if size - j >= 8:
                                    ...
                                    contents += "mov al, [rbp-" + str(8 + location + size - j - 8) + "]\n"
                                    contents += "mov [rsp+" + str(j) + "], al\n"
                                    j += 1
                                else:
                                    print("sizing error")
                        ...
                        if isinstance(function.instructions[index0 + 1], PointerNode):
                            ...
                            while j < size:
                                if size - j >= 8:
                                    ...
                                    contents += "mov al, [_" + instruction.name + "+" + str(j) + "]\n"
                                    contents += "mov [rsp+" + str(j) + "], al\n"
                                    j += 1
                                else:
                                    print("sizing error")
                    ...
                    if instruction.name in function.locals:
                        ...
                        while i < size:
                            if size - i >= 8:
                                ...
                                contents += "mov al, [rsp+" + str(i) + "]\n"
                                contents += "mov [rbp-" + str(8 + size + location - i - 8) + "], al\n"
                                i += 1
                            else:
                                print("sizing error")
                                exit()
                        contents += "add rsp, " + str(size) + "\n"
                    else:
                        size = get_size_linux_x86_64(globals[instruction.name], ast)
                        #size = (((size) + 7) & (-8))
                        i = 0
                        while i < size:
                            if size - i >= 8:
                                ...
                                contents += "mov al, [rsp+" + str(i) + "]\n"
                                contents += "mov [" + "_" + instruction.name + "+" + str(i) + "], al\n"
                                i += 1
                            else:
                                print("sizing error")
                                exit()
                    ...
                    while i < size:
                        if size - i >= 8:
                            ...
                            j -= 1
                            contents += "mov al, [rsp+" + str(size - i - 1) + "]\n"
                            contents += "mov [rbp+" + str(16 + params_size - size - i + size_rounded) + "], al\n"
                            i += 1
                        else:
                            print("sizing error")
                            exit()
                    ...
                    contents += "mov al, [rsp]\n"
                    contents += "add rsp, 1\n"
                    contents += "cmp al, " + str(1 if instruction.wants_true else 0) + "\n"
                    contents += "je target_" + str(instruction.id) + "\n"
                elif isinstance(instruction, JumpNode):
                    contents += "jmp target_" + str(instruction.id) + "\n"
                elif isinstance(instruction, ZeroNode):
                    size = get_size_linux_x86_64(instruction.type, ast)
                    contents += "sub rsp, " + str(size) + "\n"
                    j = 0
                    while j < size:
                        if size - j >= 8:
                            ...
                            contents += "mov [rsp+" + str(j) + "], word 0\n"
                            j += 2
                        elif size - j >= 1:
                            contents += "mov [rsp+" + str(j) + "], byte 0\n"
                            j += 1
                        else:
                            print("sizing error")
                            exit()
